...
<form method="POST" action="<?php echo U('Assistente/add');?>">
<TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
  <THEAD>
<TR>
  <TH valign="top"><span class="red">*</span><label for="keyword">姓名：</label></TH>
  <TD width="320"><input type="input" class="px" id="name" value="<?php echo ($staff_info["name"]); ?>" name="name" style="width:300px" ></TD>
  </TR>
  </THEAD>
  <TBODY>
  <TR>
  	<TH><span class="red">*</span>用户名：</TH>
  	<TD><input type="input" class="px" id="username" name="username"  value="<?php if($staff_info["username"] != ''): echo ($staff_info["username"]); endif; ?>"  style="width:300px" autocomplete="off"/><td><span id="errorinfo" style="color:red;"></span></td></TD>
	</TR>
  <TR>
   ...
   <TR>
  	<th>电话：</th>
  	<td><input type="input" class="px" id="tel" value="<?php echo ($staff_info["tel"]); ?>"  name="tel" style="width:300px" /></td>
  	</TR>
	<!--选择粉丝-->
	<tr>
		<th>选择店员微信openid：</th>
		<td colspan="2">
			<input name="wecha_id" id="fans_id" class="px" value="<?php echo ($staff_info["wecha_id"]); ?>" style="width:250px;"/>
   ...
   <TR>
  	...
  	<td>
	<input type="checkbox" name="pcorwap[]" value="1" <?php if($staff_info["pcorwap"] != 2 || $staff_info["pcorwap"] == ''): ?>checked<?php endif; ?>/>电脑端&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="checkbox" name="pcorwap[]" value="2" <?php if($staff_info["pcorwap"] != 1 || $staff_info["pcorwap"] == ''): ?>checked<?php endif; ?>/>手机端
	<!--select name="pcorwap" style="width:120px;" multiple="multiple">
	<option value="1" <?php if($staff_info["pcorwap"] == 1 || $staff_info["pcorwap"] == 3): ?>selected='selected'<?php endif; ?>>电脑端</option>
	<option value="2" <?php if($staff_info["pcorwap"] == 2 || $staff_info["pcorwap"] == 3): ?>selected='selected'<?php endif; ?>>手机端</option>
	</select-->&nbsp;&nbsp;&nbsp;(至少选一个类型,可以多选)
	</td>
  	</TR>
	<tr id="select_func">
	...
	<?php if($staff_info != ''): if(is_array($func)): $i = 0; $__LIST__ = $func;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$f): $mod = ($i % 2 );++$i; if($i%6 == 1): ?><tr><?php endif; ?>
	<td title="<?php echo ($f["name"]); ?>">
		<input type="checkbox" style="cursor:pointer" name="func[]" value="<?php echo ($f["funname"]); ?>" id="func_<?php echo ($f["id"]); ?>" <?php if(in_array(($f['funname']), is_array($staff_info['func'])?$staff_info['func']:explode(',',$staff_info['func']))): ?>checked<?php endif; ?> /> 
		<label for="func_<?php echo ($f["id"]); ?>" style="cursor:pointer"><?php echo ($f["name"]); ?></label> 
	</td>
	<?php if($i%6 == 0 || $i == count($func)): ?></tr><?php endif; endforeach; endif; else: echo "" ;endif; ?>
	<?php else: ?>
	<?php if(is_array($func)): $i = 0; $__LIST__ = $func;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$f): $mod = ($i % 2 );++$i; if($i%6 == 1): ?><tr><?php endif; ?>
	<td title="<?php echo ($f["name"]); ?>">
		<input type="checkbox" style="cursor:pointer" name="func[]" value="<?php echo ($f["funname"]); ?>" id="func_<?php echo ($f["id"]); ?>"/> 
		<label for="func_<?php echo ($f["id"]); ?>" style="cursor:pointer"><?php echo ($f["name"]); ?></label> 
	</td>
	<?php if($i%6 == 0 || $i == count($func)): ?></tr><?php endif; endforeach; endif; else: echo "" ;endif; endif; ?>
	</table>							
	</div>	
	</td>
	</tr>
   <tr>
  	<th></th>
	<input type="hidden" name="id" value="<?php echo ($staff_info["id"]); ?>" />
  	<td><button type="submit"  name="button"  class="btnGreen left" >保存</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<a href="<?php echo U('Assistente/add',array('id'=>$_GET['id'],'token'=>$token));?>" class="btnGreen">取消</a></td>
  	<td>&nbsp;</td>
  	<td>&nbsp;</td>
  	</tr>
  </TBODY>
