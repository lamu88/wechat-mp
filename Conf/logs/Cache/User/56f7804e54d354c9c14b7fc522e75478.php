...
<ul>
  ...
  <li>
  <a href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo ($f_qq); ?>&site=qq&menu=yes"  class="gold" title="查看资金" style="background-color:#60D295" >
      <p><strong>官方QQ号</strong></p>
      <p><?php echo ($f_qq); ?></p>
  </a>
  </li>
...
<?php if($usertplid == 2): ?><script src="<?php echo ($staticPath); ?>/tpl/static/new/js/jquery-2.1.1.js"></script>
      <div class="ibox-content">
          <table class="table table-hover">
           ...
           <TBODY>
            ...
            <TD>
        <?php if($vo['qr'] == ""){?>
        <p><a href="<?php echo U('Function/index',array('id'=>$vo['id'],'token'=>$vo['token']));?>" ><img src="<?php echo ($vo["headerpic"]); ?>" width="40" height="40"></a></p><p style="<?php if($vo['error'] != 0){echo "color:red";}?>"><?php echo ($vo["wxname"]); ?></p>
        <?php }else{?>
        <p><a href="<?php echo U('Function/index',array('id'=>$vo['id'],'token'=>$vo['token']));?>" >
        <div class="cateimg">
        <img src="<?php echo ($vo["headerpic"]); ?>" width="40" height="40" style="width:40px;height:40px" class="cateimg_small">
        <img src="<?php echo ($vo["qr"]); ?>" class="cateimg_big" style="z-index:1;" />
        </div>
        </a></p><p style="<?php if($vo['error'] != 0){echo "color:red";}?>"><?php echo ($vo["wxname"]); ?></p>
        <?php }?>
      </TD>
  ...
  <?php if($vo['qr'] != '' || $topdomain == 'pigcms.cn'){?>
      <a class="btnGrayS" <?php if($usertplid == 1): ?>href="<?php echo U('Function/show',array('id'=>$vo['id'],'token'=>$vo['token']));?>" <?php elseif($usertplid == 2): ?>href="<?php echo U('Function/show',array('id'=>$vo['id'],'token'=>$vo['token']));?>"<?php else: ?>href="<?php echo U('Function/index',array('id'=>$vo['id'],'token'=>$vo['token']));?>"<?php endif; ?>class="btnGreens" >功能管理</a>
    <?php }else{?>
...
<div class="msgWrap">
  ...
  <!--底部-->
    ...
    <!--ad start-->
    <?php if($usertplid != 2): if($thisAD): ?><div id="ad1" style="width: 100%; height: 100%; position: fixed; z-index: 1997; top: 0px; left: 0px; overflow: hidden;"><div style="height: 100%; background: none repeat scroll 0% 0% rgb(0, 0, 0); opacity: 0.65;filter:alpha(opacity=65);">
        ...
        success:function(sta){
          if(sta.length<'4'){//判断是否超时
            ...
            if(<?php echo ($usertplid); ?>==1 || <?php echo ($usertplid); ?>==2){//判断选择的模板
              if(obj=='1'){
                location="<?php echo U('Function/show');?>&id="+id+"&token="+token;
              }else{
