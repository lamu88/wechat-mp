...
<div class="content">
	...
	<div class="cLine">
		<div class="pageNavigator left"> <a href="<?php echo U('Sentiment/set',array('token'=>$token));?>" title="添加活动" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加活动</a></div>
	...
	<div class="msgWrap">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td>
						<?php
 if($vo['state'] == 0){ echo "<span style='color:red'>已关闭</span>"; }elseif($vo['state'] == 1){ echo "<span style='color:blue'>未开始</span>"; }elseif($vo['state'] == 2){ echo "<span style='color:blue'>已结束</span>"; }elseif($vo['state'] == 3){ echo "<span style='color:green'>活动中</span>"; } ?>
					</td>
					<td><?php echo ($vo["share_count"]); ?></td>
					<td class="norightborder">
						<a href="<?php echo U('Sentiment/rank',array('id'=>$vo['id'],'token'=>$token));?>">魅力排行榜</a>
						<a href="<?php echo U('Sentiment/set',array('id'=>$vo['id'],'token'=>$token));?>">修改</a>
						<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Sentiment/del',array('id'=>$vo['id'],'token'=>$token));?>');">删除</a></td>
					</td>
