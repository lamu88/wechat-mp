...
<link type="text/css" rel="stylesheet" href="./tpl/static/tpl/1352/css/base.css">
...
<section id="container">
    <ul class="oh menu71">
      <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i < 7): ?><li><a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>"><img src="<?php echo ($vo["img"]); ?>"><span><?php echo ($vo["name"]); ?></span></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?> 
    </ul>
<?php if($homeInfo['switch'] == 0): ?><section class="ap oh carouselBox carouselBox3" id="carouselBox" ontouchstart="touchStart(event)" ontouchmove="touchMove(event);" ontouchend="touchEnd(event);" style="overflow: hidden;">
...
<div style="height: 34px;line-height: 34px;font-size: 12px;background: #e3e2dd;text-align: center;color: #999;">
  <p>
  <?php if($iscopyright == 1 && $homeInfo["copyright"] != ''): echo ($homeInfo["copyright"]); ?>
  <?php elseif($iscopyright == 1 && $homeInfo["copyright"] == ''): ?>
  技术支持：<?php echo ($tpl['wxname']); ?>
  <?php else: ?>
