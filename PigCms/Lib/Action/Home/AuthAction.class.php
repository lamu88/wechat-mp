...
class AuthAction extends BaseAction {	
	...
	private $_status = array(
			...
			40202 => array('status'=>40202, 'message'=>'登录失败,账户不存在'),
	...
	private $_callback = array(
		1 => array(
			'pay' => '/wap/otherPay.php?',
			'auth' => '/wap/otherAuth.php?',
			'share' => '/wap/otherShare.php?',
			'follow' => '/wap/otherFollow.php?',
		),
		2 => array(
			'pay' => '/wap.php?c=Wxapp&a=pay&',
			'auth' => '/wap.php?c=Wxapp&a=redirect&',
			'share' => '/wap.php?c=Wxapp&a=share&',
			'follow' => '/wap.php?c=Wxapp&a=check_follow&',
		),
	);
	
	private $_type = array('weidian'=>1, 'o2o'=>2); // 对接来源
	public $_accessListAction = array(
				'bargain', 'seckill', 'crowdfunding', 'unitary', 'cutprice', 'lottery', 'red_packet', 'guajiang', 'jiugong', 'luckyfruit', 'goldenegg',
				'voteimg', 'custom', 'card', 'game', 'live', 'research', 'forum', 'autumn', 'helping','cointree','donation','collectword','sentiment','frontpage','test','punish','shakelottery','yousetdiscount','popularity','problem','seniorscene'
			);
	
	private $_allowModel = array(
		'select' => array(
			'Bargain', 'SeckillAction', 'Crowdfunding', 'Unitary', 'Cutprice', 'RedPacket', 'Lottery',
			'Voteimg', 'CustomSet', 'Cards', 'Games', 'Research', 'ForumConfig', 'Live', 'Helping',
			'Guajiang', 'Jiugong', 'LuckyFruit', 'GoldenEgg',
			'CashierActivity', 'BargainKanuser', 'SeckillBaseShop',	'CrowdfundingOrder', 'UnitaryLucknum', 'CutpriceOrder','Cointree','Donation','Collectword','Sentiment','Frontpage_action','Test','Punish','Shakelottery','Yousetdiscount','Popularity','Problem_game','SeniorScene'
		),
		'update' => array(
			'Nothing', 'Bargain', 'Seckill', 'Crowdfunding', 'Unitary', 'Cutprice', 'RedPacket', 'Lottery',
			'Voteimg', 'CustomSet', 'Cards', 'Games', 'Research', 'ForumConfig', 'Live', 'Helping',
			'Guajiang', 'Jiugong', 'LuckyFruit', 'GoldenEgg','Cointree','Donation','Collectword','Sentiment','Frontpage_action','Test','Punish','Shakelottery','Yousetdiscount','Popularity','Problem_game','SeniorScene'
		),
	...
	// 活动转换成订单
	private $_activityToOrder = array(
		'Seckill' => 'SeckillBook',	//秒杀订单
	);
	...
	public function signup() {
		...
		if($this->_signVeryfy($_POST, $sign)){
			//创建cms平台账号
			$user 	= array(
					'username' 		=> $_POST['username'].'_'.$_POST['type'],
					'password' 		=> md5($_POST['username'].$_POST['userid']),
					'createtime' 	=> time(),
					'createip' 		=> get_client_ip(),
					'gid' 			=> 4,	// 用户组
					'usertplid'		=> 1,	// 主题
					'viptime' 		=> strtotime('+20 year'),
					'status' 		=> 1,
					'is_syn' 		=> $is_syn,
					'openid' 		=> $_POST['userid'],
					'source_domain' => $_POST['domain'],
			);
			...
			if(empty($wxuser)){
				$wxuser = array(
						'uid' 		=> $user_id,
						'routerid' 	=> $_POST['wxuserid'],
						'wxname' 	=> $_POST['type'].'_'.substr(md5($_POST['wxuserid']),8,10),
						'token' 	=> $token,
						'winxintype'=> $_POST['wxtype'], 	//公众号类型
						'oauth' 	=> 0,
						'is_syn' 	=> $is_syn,
						'createtime'=> time(),
				);
				...
				$result = M('Wxuser')->where(array('uid'=>$user_id,'token'=>$token))->save($save);
			}
	...
	public function access() {
		...
		if($this->_signVeryfy($_GET, $sign)) {
			// 验证过来的token 和域名
			if($action == 'seniorScene'){
				redirect($this->siteUrl.U('User/'.ucfirst($action).'/highLive', array('token'=>$token)));
			}else{
	...
	public function select() {
		$result = $this->_db('select');
		$_POST['option'] = array_intersect_key($_POST['option'], array_flip($this->_allowDbMethod));
		$result['model'] = $this->_getModel($result['model']);
	...
	public function share() {
		if ($this->isAgent) {
			...
			$agent_conf 	= D('Agent')->where(array('id'=>$this->agentid))->find();
			$wxuser['appid'] 	= $agent_conf['appid'];
			$wxuser['appsecret'] 	= $agent_conf['appsecret'];	
		} else {
			$wxuser['appid'] 	= C("appid");
			$wxuser['appsecret'] 	= C("secret");
		}
	...
	private function _db($option) {	//不进行session验证
		if (IS_POST) {
			...
			if (!in_array($model, $this->_allowModel[$option])) {
				print_r($model);
				print_r($this->_allowModel);
				exit(json_encode($this->_status[40301]));
			}
			if ('select' != $option) {	// select 不再验证token  可能会分出去 
	...
	private function _getModel($type) {
		switch ($type) {
			case 'Lottery':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '1';
				break;
			case 'Guajiang':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '2';
				break;
			case 'Jiugong':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '10';
				break;
			case 'LuckyFruit':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '4';
				break;
			case 'GoldenEgg':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '5';
				break;
			case 'Autumn':
				$model 	= 'Lottery';
				$_POST['option']['where']['type'] 	= '9';
				break;
			default:
				$model 	= $type;
		}	
	...
	private function _login() {
		...
		$pwd=md5($uname.$userid); // 等于注册时的密码
		// 验证是否允许登录
		$res=$db->where(array('username'=>$uname.'_'.$type,'openid'=>$_POST['userid'], 'is_syn'=>$this->_type[$type]))->find();
		
		if($res&&($pwd===$res['password'])){
			//公众号账户是否存在
			$wxuser = D('Wxuser')->where(array('uid'=>$res['id']))->find();
			if(!$wxuser) exit(json_encode($this->_status[40202]));
