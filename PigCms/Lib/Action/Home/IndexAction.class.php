...
class IndexAction extends BaseAction{
	...
	protected function _initialize(){
		...
		if($agentid == 0){
			...
			$zdydh=S('zdydh');
			$this->assign('copyright',C('copyright'));
		}else{
			$agent = M('Agent');
	...
	public function index(){
		...
		if(C('server_topdomain')=='pigcms.cn'){
			...
			$top_arr=M('Funclass')->order("id desc")->limit(9)->field(true)->select();
			$this->assign('top_arr',$top_arr);
		}
		$reasonIist=M('Reason')->where(array('status'=>1))->order('id desc')->field(true)->select();
		$this->assign('reasonIist',$reasonIist);
		if (!isset($_SESSION['closeAD'])){
			$ads=M('Renew')->where(array('status'=>1))->order('id DESC')->limit(0,5)->select();
			$thisAD=$ads[rand(0,4)];
			if ($thisAD['imgs']){
		...
		if(session('displayid')){
			$displayid=session('displayid')==1 ? '' : session('displayid');
			$this->display($this->home_theme.':Index:'.ACTION_NAME.$displayid);
		}else{
			$displayid=M('Extendset')->limit(1)->getField('cloose_display');
			$displayid=$displayid==1 ? '' : $displayid;
			$this->display($this->home_theme.':Index:'.ACTION_NAME.$displayid);
		}
	...
	public function fc(){
		...
		if(C('server_topdomain')=='pigcms.cn'){
			if($displayid==2){
				$funClass=M('Funclass')->order('id desc')->field(true)->select();
				$this->assign('funclass',$funClass);
			}else{
				$db=D('Classify');
	...
	function common(){
		...
		if(C('server_topdomain')=='pigcms.cn'){
			if($displayid==2){
				$class=M('Caseclass')->order('id DESC')->field(true)->select();
				...
				$cases=M('Case')->where(array('status'=>1,'class'=>$classid))->order('id DESC')->field(true)->select();
			}else{
				$db=D('Classify');
		    		$dbs=D('Img');
			...
			if($displayid==2){
				...
				$cases=M('Case')->where($whe)->order('id DESC')->select();
			}else{
				$where['status']=1;
				if (C('agent_version')){
					$where['agentid']=$this->agentid;
				}
				$class = D('Caseclass')->where($where)->field(true)->select();
	...
	public function reg(){
		...
		$this->assign('business',$data);
		if( C('server_topdomain')=='pigcms.cn' ){
			$this->assign('server_topdomain',1);
		}
		if(session('displayid')){
			$displayname=session('displayid')==1 ? 'login' : ACTION_NAME.session('displayid');
			$this->display($this->home_theme.':Index:'.$displayname);
		}else{
			$displayid=M('Extendset')->limit(1)->getField('cloose_display');
			$displayname=empty($displayid)||$displayid==1 ? 'login' : ACTION_NAME.$displayid;
			$this->display($this->home_theme.':Index:'.$displayname);
		}
	}
	/*
	...
	*/
	...
	public function ajax_dan(){
		...
		foreach($res as $k => $v){
			...
			if($k=='desc'  && !empty($v)){
				if(mb_stripos('&nbsp;'.$v,'小猪CMS',0,'utf-8')){
					$res[$k]=str_ireplace('小猪CMS',C('site_name'),$v);
				}
			}
