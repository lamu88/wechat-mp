...
class TokenAction extends BackAction{
	...
	public function edit(){
		...
		if(IS_POST){
			if($_POST['phone'] != ""){
				$reg= '/^0{0,1}1[0-9]{10}$/i';
				if(!preg_match($reg, $_POST['phone'])){
					$this->error('手机号格式错误');
				}
				if(M('verifier_message')->where(array('token'=>$_POST['token']))->find()){
					M('verifier_message')->where(array('token'=>$_POST['token']))->setField('verifier_phone',$_POST['phone']);
				}
			}
			...
			$message = M('verifier_message')->where(array('token'=>$wxuser['token']))->find();
			if($message){
