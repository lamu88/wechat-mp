...
class UseAction extends BackAction{
    ...
    public function index(){
        ...
        if ($this->_get('is_what') !== 'public_hao') {
            foreach ($data as $key => $value) {
                $value['controller'] = $this->strToChinese($value['controller'], $value['module']);
                $data[$key] = $value;      
            }
            $this->assign('pc','1');
        }
        if ($group == 'token') {
            ...
            foreach ($data as $key => $value) {
                if(eregi("[^\x80-\xff]",$this->strToChinese($value['controller'], $value['module']))){
                    unset($data[$key]);
                }
            }
            $bing = array_slice($data,0,10);
            if ($bing) {
                foreach ($bing as $key => $value) {
                    $wapJson .= "{value:".$value['count'].", name:'".$this->strToChinese($value['controller'], $value['module'])."'},";
                }
                $wapJson = rtrim($wapJson,',');
                $pie = array('series' => "$wapJson");
            }else{
    ...
    public function strToChinese($str,$module){
