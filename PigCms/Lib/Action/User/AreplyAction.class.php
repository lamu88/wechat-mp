...
class AreplyAction extends UserAction{
	...
	public function insert(){
		...
		$res=$db->where($where)->find();
		if($res==false){
			$where['content']=html_entity_decode($_POST['content']);
	...
	public function advanceindex(){
		...
		foreach ($list as $key => $value) {
			if($value['reply_type'] == 5 && $value['relevance_id'] == ""){
				$list[$key]['status'] = 1;
			}elseif($value['reply_type'] == 5 && $value['relevance_id'] != ""){
	...
	public function advanceadd(){
		if(IS_POST){
			...
			//多个时间段
			}elseif(count($_POST['start_time']) > 1 && count($_POST['end_time']) > 1){
		...
		if($_GET['id'] != ""){
			...
			if(5 == $set['reply_type']){
				if(strpos($set['relevance_id'],',') !== false){
					...
					$mlist = M('img')->where(array('id'=>array('in',$set['relevance_id'])))->field('id,text,pic,info,url,title')->order('field(id,'.$set['relevance_id'].')')->select();
					$this->assign('end_img',$mlist[0]);
					array_shift($mlist);
					$this->assign('mlist',$mlist);
				}
			}elseif(1 == $set['reply_type'] || 7 == $set['reply_type']){
	...
	public function AddimgReply($data){
		if($data['r_type'] == 'select'){ //单图文选择模式
			...
			if($data['id'] == ""){
				...
				}elseif(count($data['start_time']) > 1 && count($data['end_time']) > 1){
				...
				if($insert){
					if($data['original_id'] != ""){//编辑的时更换回复类型
						M("subscribe_reply")->where(array('id'=>$data['original_id']))->delete();
					}
					$this->success('单图文消息添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
				}else{
				...
				if($update){
					$this->success('单图文消息编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
				}else{
			...
			if($data['id'] == ""){
				...
				if($updateImg !== false && $updateReply !== false){
					$this->success('单图文编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
				}else{
					$this->error('单图文编辑失败');
				}
			}
		...
		if($data['id'] == ""){
			...
			if($addImgId){
				...
				if($insert){
					...
					$this->success('外链图文消息添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
				}else{
			...
			$updateReply = M('subscribe_reply')->where(array('id'=>$data['id']))->save($data);
			if($updateImg !== false && $updateReply !== false){
				$this->success('外链图文消息编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
	...
	private function AddmultiimgReply($data){
		...
		if($data['id'] == ""){
			...
			if($insert){
				...
				$this->success('多图文添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('多图文添加失败');
			}
		}else{
			...
			$update = M('subscribe_reply')->where(array('id'=>$data['id']))->save($data);
			if($update){
				$this->success('多图文编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('多图文编辑失败');
			}
		}
	...
	public function AddcardReply($data){
		...
		if($data['id'] == ""){
			...
			if($insert){
				...
				$this->success('回复卡券添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('回复卡券添加失败');
			}
		}else{
			...
			$update = M('subscribe_reply')->where(array('id'=>$data['id']))->save($data);
			if($update){
				$this->success('回复卡券编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('回复卡券编辑失败');
			}
		}
	...
	public function AddhbReply($data){
		...
		if($data['id'] == ""){
			...
			if($insert){
				...
				$this->success('回复红包添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('回复红包添加失败');
			}
		}else{
			...
			$update = M('subscribe_reply')->where(array('id'=>$data['id']))->save($data);
			if($update){
				$this->success('回复红包编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('回复红包编辑失败');
			}
		}
	...
	private function AddtitleReply($data){
		...
		$data['title'] = preg_replace("/(\015\012)|(\015)|(\012)/", "\n",$_POST['text_title']);
		$data['add_time'] = $_SERVER['REQUEST_TIME'];
		if($data['id'] == ""){
			if(count($data['start_time']) == 1 && count($data['end_time']) == 1){
				$data['token'] = $this->token;
			...
			if($insert){
				...
				$this->success('文本回复添加成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('文本回复添加失败');
			}
		}else{
			...
			$update = M('subscribe_reply')->where(array('id'=>$data['id']))->save($data);
			if($update){
				$this->success('文本回复编辑成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}else{
				$this->error('文本回复编辑失败');
			}
		}
   ...
   public function select_multiReply(){
	...
	public function select_card(){
		C('TOKEN_ON',false);
		$name  	= $this->_request('name','trim');
		$where 	= array('token'=>$this->token,'is_weixin'=>1,'is_check'=>1,'is_delete'=>0,'total'=>array('gt',0));
		if($name){
			$where['title'] = array('like','%'.$name.'%');
		}
	...
	public function del(){
		...
		if(!empty($exists)){
			...
			if($del){
				$this->success('删除成功',U('Areply/advanceindex',array('token'=>$this->token)));exit;
			}
		}else{
			$this->error('不存在该删除项');exit;
		}
	}
	//ajax验证时间合法性
	public function Timevalid(){
		...
		if(!empty($_POST['start_time_arr']) && !empty($_POST['end_time_arr'])){
			...
			foreach ($start_time_arr as $k => $v) {
				...
				if(($start_timestamp_pre <= $start_timestamp && $start_timestamp < $end_timestamp_pre) || ($start_timestamp_pre < $end_timestamp && $end_timestamp <= $end_timestamp_pre) || ($start_timestamp_pre > $start_timestamp && $end_timestamp_pre < $end_timestamp)){
					exit('time_post_fail');
				}
			}
		}
		...
		if(!empty($last_data)){
			if($id != "" && $type =='edit'){//修改
				...
				foreach($last_data as $key => $value){
					//修改的时间与已经存在的时间做对比，并且已存在的时间不包含被修改的这条
					if($value['start_time'] != $update_data['start_time'] && $value['end_time'] != $update_data['end_time']){
						...
						if(($vs_timestamp <= $start_timestamp && $start_timestamp < $ve_timestamp) || ($vs_timestamp < $end_timestamp && $end_timestamp <= $ve_timestamp) || ($vs_timestamp > $start_timestamp && $ve_timestamp < $end_timestamp)){
							exit('time_fail');
						}else{
							continue;
						}
	...
	public function multititle(){
		$ids = trim($_GET['ids'],',');
		$imgs = M('img')->where(array('id'=>array('in',$ids)))->field('id,title,pic')->select();
		$this->assign('list',$imgs);
		$this->display();
	}
	/*function getImgText(){
		$id = (int)$_POST['id'];
		$text = M('img')->where(array('id'=>$id))->getField('text');
		exit($text);
	}*/
	public function test1(){
		$img_list = M('subscribe_reply')->where(array(C('DB_PREFIX').'subscribe_reply.id'=>array('in',array(3,2,1))))->join(C('DB_PREFIX').'subscribe_img on '.C('DB_PREFIX').'subscribe_reply.id = '.C('DB_PREFIX').'subscribe_img.rid')->field('title,reply_content,reply_pic,jump_url')->select();
		$multi = array();
		foreach ($img_list as $key => $value) {
			array_push($multi,$value);
		}
		$data  = array(array($multi),'news');//多图文
	}
