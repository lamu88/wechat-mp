...
class CoinTreeAction extends UserAction{
	...
	public function add_action(){
		if(IS_POST){
			...
			if((int)$data['everydaytimes'] > (int)$data['totaltimes']){
			...
			if(strlen($data['action_desc']) > 2400){
			...
			//数量、图片、说明同时满足时才入库
			if($this->_post('second_nums','intval') >0 &&  $this->_post('second_img','trim') != "" && $this->_post('second_prize','trim') != ""){
			...
			if($this->_post('third_nums','intval') >0 &&  $this->_post('third_img','trim') != "" && $this->_post('third_prize','trim') != ""){
			...
			if($this->_post('fourth_nums','intval') >0 &&  $this->_post('fourth_img','trim') != "" && $this->_post('fourth_prize','trim') != ""){
			...
			if($this->_post('fifth_nums','intval') >0 &&  $this->_post('fifth_img','trim') != "" && $this->_post('fifth_prize','trim') != ""){
			...
			if($this->_post('sixth_nums','intval') >0 &&  $this->_post('sixth_img','trim') != "" && $this->_post('sixth_prize','trim') != ""){
			...
			if(!$id){
				...
				if($action_id && $add_prize){
					...
					$this->success('添加摇钱树活动成功',U('CoinTree/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('添加摇钱树活动失败');exit;
				}
			}else{
				$stat = true;
				$update_action = M('cointree')->where(array('id'=>$id))->save($data);
				//要全等于false,因为没修改时返回受影响记录数为0
				...
				if($stat){
					...
					$this->success('修改摇钱树活动成功',U('CoinTree/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('修改摇钱树活动失败');exit;
				}
			}			
		}
		if($_GET['id'] != ""){
	...
	public function prizerecord(){
		C('TOKEN_ON',false);
		$token = $this->_request('token','trim');
		$action_id =  $this->_request('id','intval');
	...
	public function editprizerecord(){
		if(IS_POST){
			...
			if(!empty($exists)){
				...
				if($update){
					$this->success('编辑成功',U('CoinTree/prizerecord',array('token'=>$exists['token'],'id'=>$exists['cid'])));exit;
				}else{
					$this->error('编辑失败');exit;
				}
			}else{
				$this->error('不存在该编辑项');exit;
			}
		}
		if($_GET['id']){
	...
	public function delprizerecord(){
		...
		if(!empty($exists)){
			...
			if($del){
				$this->success('删除成功',U('CoinTree/prizerecord',array('token'=>$this->token,'id'=>$exists['cid'])));exit;
			}
		}else{
	...
	public function del_action(){
		...
		if(!empty($exists)){
			...
			if($del){
				...
				S($this->token.'_'.$id.'_cointree',null);
				$this->success('删除成功',U('CoinTree/index',array('token'=>$this->token)));exit;
			}
		}else{
			$this->error('不存在该删除项');exit;
		}
