...
class FrontPageAction extends UserAction{
	...
	public function addaction(){
		if(IS_POST){
			...
			if($data['keyword'] == "" && $this->wxuser['is_syn'] != 2){
			...
			if($data['remind_word'] == ""){
				$this->error('广告语不能为空');
			}elseif(strlen($data['remind_word']) > 45){
			...
			if((int)$data['day_create'] > (int)$data['total_create']){
			...
			if(!empty($configlist)){
				foreach($configlist as $val){
					if(($val['expires_in'] > time() - $val['addtime']) && $data['status'] == 1){
						$data['status'] = 1;
						break;
					}else{
						$data['status'] = 0;
			...
			if($data['id'] == ""){
				...
				if($action_id){
					...
					$this->success('添加我要上头条活动成功',U('FrontPage/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('添加我要上头条活动失败');exit;
				}
			}else{
				...
				if($stat){
					...
					$this->success('修改我要上头条活动成功',U('FrontPage/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('修改我要上头条活动失败');exit;
				}
			...
			if(empty($exists)){
				...
				if($getAccessToken['status'] == 'success'){
					...
					if($this->IsValid($getAccessToken['access_token'])){
						...
						if($addconfigure){
							$this->success('添加配置成功',U('FrontPage/listconfigure',array('token'=>$this->token)));exit;
						}else{
							$this->error('添加配置失败');
						}
					}else{
				...
				}elseif($getAccessToken['status'] == 'error'){
					$this->error('添加配置失败,'.$getAccessToken['error_description']);
				}
			}else{
				//多个公众号共用access_token,减少请求接口次数
				if($exists['token'] != $this->token){
					...
					$congfig['token'] = $this->token;
					$congfig['addtime'] = $exists['addtime'];//addtime也继承防止失效
					$addconfigure = M("frontpage_configure")->add($congfig);
					if($addconfigure){
						$this->success('添加配置成功',U('FrontPage/listconfigure',array('token'=>$this->token)));exit;
					}else{
	...
	public function delaction(){
		...
		if(!empty($exists)){
			...
			if($del){
				...
				$this->success('删除成功',U('FrontPage/index',array('token'=>$this->token)));exit;
			}
		}else{
	...
	public function updateconfigure(){
		...
		if(!empty($config)){
			...
			if($getAccessToken['status'] == 'success'){
				...
				if($updateconfigure){
					$this->success('更新访问令牌成功',U('FrontPage/listconfigure',array('token'=>$this->token)));exit;
				}else{
					$this->error('更新访问令牌失败');
				}
			}elseif($getAccessToken['status'] == 'error'){
	...
	public function delconfigure(){
		...
		if(!empty($exists)){
			...
			if($del){
				$this->success('删除成功',U('FrontPage/listconfigure',array('token'=>$this->token)));exit;
			}
		}else{
			$this->error('不存在该删除项');exit;
		}
	...
	public function adddefinenews(){
		if(IS_POST){
			...
			if($id == ""){
				...
				if($addid){
					$this->success('添加成功',U('FrontPage/definenews',array('token'=>$this->token,'news_type'=>1,'id'=>$data['cid'],'type'=>'mynews')));exit;
				}else{
				...
				if($updatenew){
					$this->success('修改成功',U('FrontPage/definenews',array('token'=>$this->token,'news_type'=>1,'id'=>$data['cid'],'type'=>'mynews')));exit;
				}else{
	...
	public function deldefinenews(){
		...
		if(in_array($id,$default_id)){
			...
			if($default_type == 'close'){
				...
				$msg = '关闭';
			}elseif($default_type == 'open' && $defaultnews_hide != ""){
			...
			if($update){
				$this->success($msg.'成功',U('FrontPage/systemnews',array('token'=>$this->token,'news_type'=>$news_type,'id'=>$cid)));exit;
			}else{
				$this->error($msg.'失败');exit;
			}
		}else{
			$id = (int)$_GET['id'];
		}
		...
		if(!empty($exists)){
			...
			if($del){
				...
				if($news_type == 1){
					$this->success('删除成功',U('FrontPage/definenews',array('token'=>$this->token,'news_type'=>$news_type,'id'=>$cid,'type'=>'mynews')));exit;
				}else{
					$this->success('删除成功',U('FrontPage/fansnews',array('token'=>$this->token,'news_type'=>$news_type,'id'=>$cid)));exit;
				}
			}
		}else{
			$this->error('不存在该删除项');exit;
	...
	public function uploadimg(){
		if(IS_POST){
			...
			if($default_img == ""){
				...
				if($add){
					$this->success('添加图片成功',U('FrontPage/systemnews',array('token'=>$data['token'],'type'=>'default','id'=>$data['cid'])));exit;
				}else{
					$this->error('添加图片失败');
				}
			}else{
				...
				if($update){
					$this->success('修改图片成功',U('FrontPage/systemnews',array('token'=>$_POST['token'],'type'=>'default','id'=>$_POST['cid'])));exit;
				}else{
					$this->error('修改图片失败');
				}
			}
	...
	private function getAccessToken($post = ''){
		if($post['apikey'] == "" && $post['secretkey'] == ""){ return false;}
