...
class HotelsAction extends UserAction 
{
	...
	public function index()
	{
		...
		$this->assign('firstrow',$this->Page->firstRow);
		$this->display();
	}
	...
	public function houseadd() {
		...
		if (IS_POST) {
			...
			if ($id && ($house = $dataBase->where(array('id' => $id, 'token' => $this->token, 'cid' => $this->_cid))->find())) {//edit
				...
				if ($dataBase->create() !== false) {
					$action = $dataBase->save();
					if ($sid != $_POST['sid']) {//改变类型
						if($_POST['is_open'] == 1){//只有开启状态才给该房间类型增加房间数
							D('Hotels_house_sort')->where(array('id' => $_POST['sid']))->setInc('houses', 1);
						}
						D('Hotels_house_sort')->where(array('id' => $sid, 'houses' => array('gt', 0)))->setDec('houses', 1);
					}else{
						if($_POST['is_open'] == 2){
							D('Hotels_house_sort')->where(array('id' => $_POST['sid'], 'houses' => array('gt', 0)))->setDec('houses', 1);
						}elseif($_POST['is_open'] == 1){
							D('Hotels_house_sort')->where(array('id' => $_POST['sid']))->setInc('houses', 1);
						}
					}
					if ($action != false) {
			...
			$list = M('Hotels_house_sort')->where(array('cid' => $this->_cid,'is_open'=>array('neq',2)))->select();
			$this->assign('list', $list);
			$this->display();
		}
	}
	...
	public function orders() 
	{
		...
		$where = "`h`.`token`='" . $this->_session('token') . "' AND `h`.`cid`='{$this->_cid}'";//array('token' => $this->_session('token'), 'cid' => $this->_cid);
		$where_count = "`token`='" . $this->_session('token') . "' AND `cid`='{$this->_cid}'";//array('token' => $this->_session('token'), 'cid' => $this->_cid);
		if ($status) {
			$where .= " AND `h`.`startdate`<='{$status}' AND `h`.`enddate`>'{$status}'";
			$where_count .= " AND `startdate`<='{$status}' AND `enddate`>'{$status}'";
// 			$where['startdate'] = array('ELT', $status);
// 			$where['enddate'] = array('GT', $status);
		}
		if ($searchkey) {
			$where .= " AND (`h`.`name` like '%{$searchkey}%' OR `h`.`tel` like '%{$searchkey}%' OR `h`.`orderid` like '%{$searchkey}%')";
			$where_count .= " AND (`name` like '%{$searchkey}%' OR `tel` like '%{$searchkey}%' OR `orderid` like '%{$searchkey}%')";
		}
		$this->assign('searchkey', $searchkey);
		$count      = $hotelOrder->where($where_count)->count();
		$Page       = new Page($count, 20);
		$show       = $Page->show();
		
		$sql = "SELECT h.*, c.reduce_cost FROM " . C('DB_PREFIX') . "hotels_order AS h LEFT JOIN " . C('DB_PREFIX') . "coupon_pay_record AS c ON h.wecha_id=c.wechat_id AND h.orderid=c.orderid AND c.from='Hotels' AND c.dateline>0 WHERE $where ORDER BY `h`.`id` DESC LIMIT " . $Page->firstRow . "," . $Page->listRows;
		$orders = D()->query($sql);
// 		echo D()->_sql();
	...
	public function orderInfo() {
		...
		if ($thisOrder = $hotelOrder->where(array('id' => $id, 'token' => $this->token, 'cid' => $this->_cid))->find()) {
			if (IS_POST) {
				...
				$sort = M('Hotels_house_sort')->where(array('cid' => $this->_cid, 'token' => $this->token, 'id' => $thisOrder['sid']))->find();
				$thisOrder['housename'] = isset($sort['name']) ? $sort['name'] : '';
				$coupon = D('Coupon_pay_record')->where(array('orderid' => $thisOrder['orderid'], 'token' => $this->token, 'wechat_id' => $thisOrder['wecha_id'], 'from' => 'Hotels', 'dateline' => array('gt', 0)))->find();
