...
class ImgBaseAction extends UserAction{ 
	...
	public function index(){
		...
		$_GET = arr_htmlspecialchars($_GET);
		$_POST = arr_htmlspecialchars($_POST);
	...
	public function del(){
		...
		if(D('Img')->where($where)->delete()){
			...
			M('subscribe_reply')->where(array('reply_type'=>1,'relevance_id'=>$where['id']))->delete();
			$list = M('subscribe_reply')->where("token = '".$this->token."' AND reply_type = 5")->select();
			foreach ($list as $key => $value) {
				if(strpos($value['relevance_id'],$where['id'].',') !== false){
					$value['relevance_id'] = str_replace($where['id'].',','',$value['relevance_id']);
					M('subscribe_reply')->where(array('id'=>$value['id']))->save(array('relevance_id'=>$value['relevance_id']));
				}
			}
	...
	public function insert(){
		if($_POST['dashang'] == 1 && $_POST['dashang_type'] == 2 && strpos($_POST['info'],'//dashang//') === FALSE){
			$this->error('您选择了“打赏后查看全文”，请在“图文详细页内容”中在要隐藏的内容前加上“//dashang//”。');exit;
		}
        if($_POST['dashang'] == 1 && $_POST['dashang_price_type'] == 1 && !($_POST['dashang_price_min']) >= 0 && !($_POST['dashang_price_max'] > 0)){
            $this->error('您必须填写打赏金额的最大值和最小值');exit;
        }
        if($_POST['dashang'] == 1 && $_POST['dashang_price_type'] == 2){
            $istrue = false;
            for($i = 1;$i <= 6;$i++){
                if($_POST['dashang_price_'.$i] > 0){
                    $istrue = true;    
                }  
            }
            if(!$istrue){
                $this->error('您必须至少填写一组打赏金额');exit;    
            }
		...
		$_POST['dashang_price_max'] = $_POST['dashang_price_max']*100;
		$_POST['dashang_price_1'] = $_POST['dashang_price_1']*100;
		$_POST['dashang_price_2'] = $_POST['dashang_price_2']*100;
		$_POST['dashang_price_3'] = $_POST['dashang_price_3']*100;
		$_POST['dashang_price_4'] = $_POST['dashang_price_4']*100;
		$_POST['dashang_price_5'] = $_POST['dashang_price_5']*100;
		$_POST['dashang_price_6'] = $_POST['dashang_price_6']*100;
	...
	public function syncNews($type) {
		...
		$params['post']['articles'] = array(
				array(
						...
						'author' => $this->_post('writer'),
						'digest' => $this->_post('text'),
						'show_cover_pic' => $this->_post('showpic'),
						'content' => $content,
						'content_source_url' => $this->convertLink($this->_post('url')),
				)
		);
		//if ('add' == $type) {
		//	$params['post'] = json_encode($params['post'], 256);
		//	$return = $material->addNews($params);	//新增
	...
	public function upsave(){
        ...
        if($_POST['dashang'] == 1 && $_POST['dashang_price_type'] == 2){
            $istrue = false;
            for($i = 1;$i <= 6;$i++){
                if((int)$_POST['dashang_price_'.$i] > 0){
                    $istrue = true;    
                }  
            }
            if(!$istrue){
