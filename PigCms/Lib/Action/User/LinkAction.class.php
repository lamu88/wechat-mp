...
class LinkAction extends UserAction{
	...
	//	array('module'=>'Host','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Host&a=detail&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Host'],'sub'=>1,'canselected'=>0,'linkurl'=>'','keyword'=>'','askeyword'=>1),
		...
		array('module'=>'Reply','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Reply&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Reply'],'sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'留言','askeyword'=>1),
...
/* 		if (C('server_topdomain')=='pigcms.cn'){
	...
	public function Img(){
		...
		if($ssimg == null){
			if(IS_POST){
				session($this->token.'ssimg',null);
			}else{
				$ssimg = session($this->token.'ssimg');
			}
		}
		if($ssimg){
			$where['title'] = array('like','%'.$ssimg.'%');
			session($this->token.'ssimg',$ssimg);
		}else{
			session($this->token.'ssimg',null);
		}
...
//外链小工具
	...
	public function OutsideLink($display=true){
		//处理小工具数组文件
	...
	public function outsideLinkDetail($display=true){
		$c = $this->_get('c');
