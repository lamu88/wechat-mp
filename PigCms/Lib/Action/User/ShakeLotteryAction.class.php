...
class ShakeLotteryAction extends UserAction{
	...
	public function _initialize(){
		...
		if(isset($_GET['type'])) $this->source = e($_GET['type']);
	}
	//摇一摇中奖首页
	...
	public function AddShakeLottery(){
		if(IS_POST){
			...
			if($_POST['keyword'] == '' && $this->wxuser['is_syn'] != 2){
			...
			}elseif($_POST['join_number'] == ''){
				$this->error('预计参与人数不能为空');
			}
			if((int)$_POST['everydaytimes'] > (int)$_POST['totaltimes']){
			...
			if($_POST['other_source'] == 'scene' && intval($_POST['scene_time']) < 1){
				$this->error('活动时间不能为空');
			}
			$data = array();
			$data['other_source'] = trim($_POST['other_source']);
			$data['scene_time'] = intval($_POST['scene_time']);
			...
			$data['is_amount'] = intval($_POST['is_amount']);
			$id = $this->_post('id','intval');
			if(!$id){//添加
				$data['token'] = $this->token;
				//$this->other_source_data_model($data);
				$addid = M('shakelottery')->add($data);
				if($addid){
					$this->handleKeyword($addid,'ShakeLottery',$data['keyword']);
					$this->success('活动添加成功',U('ShakeLottery/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('活动添加失败');
				}
			}else{//修改
				//$this->delete_other_source_data_model($data);
				...
				if($edit){
					...
					$this->success('活动修改成功',U('ShakeLottery/index',array('token'=>$this->token)));exit;
				}else{
					$this->error('活动修改失败');
				}
			}
	...
	public function DelShakeLottery(){
		...
		if($del){
			...
			S($this->token.'_shakelottery_'.$id,null);
			$this->success('删除成功',U('ShakeLottery/index',array('token'=>$this->token)));exit;
		}else{
			$this->error('删除失败');
		}
	}
	...
	public function AddEditPrize(){
		if(IS_POST){
			...
			if(!$id){//添加
				...
				if($prizeid){
					$this->success('奖品添加成功',U('ShakeLottery/PrizeList',array('aid'=>$data['aid'],'token'=>$this->token)));exit;
				}else{
					$this->error('奖品删除失败');
				}
			}else{//修改
				...
				if($editprize){
					$this->success('奖品修改成功',U('ShakeLottery/PrizeList',array('aid'=>intval($_POST['aid']),'token'=>$this->token)));exit;
				}else{
					$this->error('奖品修改失败');
				}
			}
	...
	public function DelPrize(){
		...
		if($del){
			$this->success('删除奖品成功',U('ShakeLottery/PrizeList',array('aid'=>$aid,'token'=>$this->token)));exit;
		}else{
			$this->error('删除奖品失败');
		}
	}
	...
	public function DelRecord(){
		...
		if($del){
			$this->success('领奖记录删除成功',U('ShakeLottery/ShakeLotteryRecord',array('aid'=>$aid,'token'=>$this->token,'type'=>$type)));exit;
		}else{
			$this->error('领奖记录删除失败');
		}
	}
	...
	public function record_details(){
		...
		if($record_details['status'] == 'SUCCESS' && $record_details['msg']['hb_type'] == 'GROUP'){
			...
			$this->assign('cid',$cid);
			$this->display();
		}elseif($record_details['status'] == 'SUCCESS' && $record_details['msg']['hb_type'] == 'NORMAL'){
	...
	public function get_nickname($openid = ''){
		...
		if(F($openid.'_'.'dhbnickname') != ""){
			return F($openid.'_'.'dhbnickname');
		}
		if(F($this->token.'_'.'directhongbao') != ""){
			$access_token = F($this->token.'_'.'directhongbao');
		}else{
			...
			F($this->token.'_'.'directhongbao',$access_token);
		}
		if($access_token){
			...
			F($openid.'_'.'dhbnickname',$classData['nickname']);
	...
	function show_array($countpage,$url,$page){  
		 $page=empty($page)?1:$page;  
		 if($page > 1){  
			$uppage=$page-1;  
