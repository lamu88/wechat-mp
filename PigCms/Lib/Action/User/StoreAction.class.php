...
class StoreAction extends UserAction{
	...
	public function addNew() 
	{
        ...
        if ($id && ($product = M('Product')->where(array('catid' => $catid, 'token' => session('token'), 'id' => $id))->find())) {
        	...
        	$productDetailData = M("Product_detail")->where(array('pid' => $id))->order('id asc')->select();
...
//	        }
	...
	public function orders()
	{
		...
		if (IS_POST) {
			...
			$key = $this->_post('searchkey');
			if ($key) {
				$where['product.truename|product.tel|product.orderid'] = array('like', "%$key%");
			} else {
				for ($i=0;$i<40;$i++){
		...
		if (isset($_GET['handled'])) {
			$where['product.handled'] = intval($_GET['handled']);
		}
		$count      = M('product_cart')->alias('product')->where($where)->count();
		$Page       = new Page($count,20);
		$show       = $Page->show();
		$orders		= $product_cart_model->alias('product')->where($where)->join( C('DB_PREFIX').'coupon_pay_record as c ON product.wecha_id = c.wechat_id AND product.orderid = c.orderid AND c.from = "Store" AND c.dateline >0')->order('time DESC')->field('c.reduce_cost,product.*')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$where['handled'] = 0;
		$unHandledCount = $product_cart_model->alias('product')->where($where)->count();
	...
	public function orderInfo()
	{
		...
		if (IS_POST){
			...
			if (intval($_POST['paid']) && empty($thisOrder['paid'])){
				...
				if (intval($thisOrder['price'])) {//支付订单打印
					$op = new orderPrint();
			...
			$coupon = D('Coupon_pay_record')->where(array('orderid' => $thisOrder['orderid'], 'token' => $this->token, 'wechat_id' => $thisOrder['wecha_id'], 'from' => 'Store', 'dateline' => array('gt', 0)))->find();
...
//			}
	...
	public function setting()
	{
		...
		if (IS_POST) {
			if ($obj) {
				unset($_POST['id']);
				if ($_POST['price'] == null || $_POST['price'] == '') $_POST['price'] = -1;
	...
	public function twitter()
	{
        ...
        if (IS_POST) {
            ...
            $list = M('Twitter_count')->alias('t')->field('t.*, user.wechaname as nickname')->join('INNER JOIN '.C('DB_PREFIX').'userinfo user  ON t.twid = user.twid and t.token=user.token' )->where("t.twid LIKE '%$key%' and t.token='$token' and t.cid=$this->_cid")->order('t.id DESC')->select();
        	...
        	$count      = M('Twitter_count')->alias('t')->field('t.*, user.wechaname as nickname')->join('INNER JOIN '.C('DB_PREFIX').'userinfo user  ON t.twid = user.twid and t.token=user.token' )->where("t.token='$token' and t.cid=$this->_cid")->count();
        	$Page       = new Page($count, 20);
        	$show       = $Page->show();
        	$list = M('Twitter_count')->alias('t')->field('t.*, user.wechaname as nickname')->join('INNER JOIN '.C('DB_PREFIX').'userinfo user  ON t.twid = user.twid and t.token=user.token' )->where("t.token='$token' and t.cid=$this->_cid")->order("t.id DESC")->limit($Page->firstRow . ',' . $Page->listRows)->select();
        }
//         $twids = '';
//         foreach ($list as $row) {
//         	$twids[] = $row['twid'];
...
//         $tmp_user = array();
//         if ($twids) {
//         	$users = D('Userinfo')->where(array('twid' => array('in', $twids)))->select();
//         	foreach ($users as $u) {
//         		$tmp_user[$u['twid']] = $u;
//         	}
//         }
...
//         }
	...
	public function detail()
	{
		...
		if ($wecha_ids) {
        	$users = D('Userinfo')->where(array('wecha_id' => array('in', $wecha_ids)))->select();
        	foreach ($users as $u) {
