...
class BargainAction extends WapAction{
	...
	public function _initialize(){
		...
		if($_GET['id'] != ''){
			...
			foreach($out_order as $outval){
				$this->m_bargain->where(array('token'=>$this->token,'pigcms_id'=>$outval['bargain_id']))->setInc('inventory',1);
	...
	public function payuserinfo(){
		if($this->is_syn == 1){
			$order = $this->m_order->where(array('token'=>$this->token,'pigcms_id'=>intval($_GET['orderid'])))->find();
			$save_order['price'] = $order['bargain_nowprice']/100;
			$save_order['orderid'] = (int)($_GET['orderid']).'bargain'.time();
			if($order['state2'] != 1){
				$save_order['state2'] = 1;
				$this->m_bargain->where(array('token'=>$this->token,'pigcms_id'=>$order['bargain_id']))->setDec('inventory',1);
				S($order['bargain_id'].'bargain'.$this->token,null);
			}
			$this->m_order->where(array('token'=>$this->token,'pigcms_id'=>intval($_GET['orderid'])))->save($save_order);
			$bargain = $this->m_bargain->where(array('token'=>$this->token,'pigcms_id'=>$order['bargain_id']))->find();
			$this->redirect("Alipay/pay",array("token"=>$this->token,"price"=>$save_order['price'],"wecha_id"=>$this->wecha_id,"from"=>"Bargain","orderid"=>$save_order['orderid'],'single_orderid'=>$save_order['orderid'],'notOffline'=>1,'product_id'=>$bargain['product_id'],'sku_id'=>$bargain['sku_id'],'id'=>$bargain['pigcms_id'],'true_orderid'=>$order['pigcms_id']));
		}else{
			$where['token'] = $this->token;
			$where['pigcms_id'] = (int)($_GET['id']);
			$bargain = S($_GET['id'].'bargain'.$this->token);
	...
	public function new_index(){
		...
		if($bargain == ''){
			$bargain = $this->m_bargain->where(array('token'=>$this->token,'pigcms_id'=>$id))->find();
			if($bargain == ''){
		...
		$this->m_bargain->where(array('token'=>$this->token,'pigcms_id'=>$id))->setInc('pv');
		...
		}elseif($bargain['is_reg'] == 2 && empty($this->fans['tel']) && $orderid == 0){
			$this->memberNotice();
		}
		if($orderid != 0){
			$myorder = $this->m_order->where(array('token'=>$this->token,'bargain_id'=>$id,'pigcms_id'=>$orderid))->find();
		...
		if($myorder != ''){
			...
			foreach($rank_all as $allkey=>$allval){
				if($allval['pigcms_id'] == $myorder['pigcms_id']){
					$myorder['rank'] = $allkey + 1;
				}
			}
			$myorder['helpcount'] = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id']))->count();
			
			$kanuser_list = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id']))->order('addtime desc')->limit(10)->select();
			...
			$kanuser_count = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id']))->count();
			...
			if($orderid != 0){
				...
				if($_GET['kanprice'] == ''){
					$xiuxi_count = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id'],'addtime'=>array('gt',$myorder['logintime'])))->count();
					$xiuxi_sum = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id'],'addtime'=>array('gt',$myorder['logintime'])))->sum('dao');
	...
	public function new_kandao(){
		...
		if($kanuser == ''){
			...
			$this->m_order->where(array('token'=>$this->token,'bargain_id'=>$id,'pigcms_id'=>$orderid))->save($save_order);
			$add_kanuser['token'] = $this->token;
	...
	public function new_kanuser_more(){
		if(IS_POST){
			...
			$kanuser_list = $this->m_kanuser->where(array('token'=>$this->token,'bargain_id'=>$id,'orderid'=>$myorder['pigcms_id']))->order('addtime desc')->limit($_POST['count'],10)->select();
			$data['kanuser_list'] = '';
