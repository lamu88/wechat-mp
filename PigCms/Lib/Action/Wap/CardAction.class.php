...
class CardAction extends WapAction{
    ...
    public function companyMap(){
        ...
        $thisCompany=$company_model->where($where)->order('id ASC')->find();
        $this->assign('thisCompany',$thisCompany);
...
/*              $owhere         = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'is_open'=>'1','cardid'=>$thisCard['id'],'start'=>array('lt',time()),'end'=>array('gt',time()));
    ...
    public function signscore(){
        ...
        $this->assign('prev_month',$month-1);
        $this->assign('next_month',$month+1);
    ...
    public function payAction(){
        ...
        if($orderid == '' && $price < 0.01){
            $this->error('请填写大于或等于0.01的正确数字');
        }
    ...
    public function payReturn(){
        ...
        if($order['paytime'] > 0){
        	$this->success('支付成功',U('Card/payRecord',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$order['cardid'])));
           //已经异步处理过了
        }else {
    ...
    public function payRecord(){
        ...
        $last = strtotime(date('Y-m-d',$start)." +1 month -1 day")+84600;
...
/*      if($m != ''){
	...
	public function jfdhhb(){
		if(IS_AJAX){
			$jfdhhb = M('member_card_jfdhhb')->where(array('token'=>$this->token,'id'=>intval($_POST['id'])))->find();
			$my_integral = M('userinfo')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->getField('total_score');
			$my_count = M('member_card_jfdhhb_record')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'jid'=>intval($_POST['id'])))->count();
			$userinfo = M('userinfo')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->find();
			if($jfdhhb['endtime'] < time()){
				$data['error'] = 1;
				$data['msg'] = '对不起，该红包已过期。';
			}elseif($this->wecha_id == '' || $userinfo == ''){
				$data['error'] = 1;
				$data['msg'] = '对不起，获取不到您的个人信息。';
			}elseif($my_count >= $jfdhhb['count']){
				$data['error'] = 1;
				$data['msg'] = '对不起，您已经兑换了'.$jfdhhb['count'].'次，不能再兑换了。';
			}elseif($my_integral < $jfdhhb['integral']){
				$data['error'] = 1;
				$data['msg'] = '对不起，您当前积分不足，请下次兑换。';
			}else{
				$config = array();
				$config['send_name'] = $jfdhhb['send_name'];
				$config['act_name'] = $jfdhhb['act_name'];
				$config['wishing'] = $jfdhhb['wishing'];
				$config['remark'] = $jfdhhb['remark'];
				$config['token'] = $this->token;
				$config['openid'] = $this->wecha_id;
				$config['money'] = $jfdhhb['hongbao']/100;
				$hb = new Hongbao($config);
				$res = json_decode($hb->send(),true);
				if(strtoupper($res['status']) == 'FAIL'){
					$data['error'] = 1;
					if(!(strpos($res['msg'],'余额不足') === FALSE)){
						$data['msg'] = '商家余额不足，要不咱兑少点？';
					}elseif(!(strpos($res['msg'],'过于频繁') === FALSE)){
						$data['msg'] = '对不起，您的操作过于频繁，请稍后再试';
					}else{
						$data['msg'] = $res['msg'];
					}
				}elseif(strtoupper($res['status']) == 'SUCCESS'){
					$add_record['token'] = $this->token;
					$add_record['wecha_id'] = $this->wecha_id;
					$add_record['cardid'] = $jfdhhb['cardid'];
					$add_record['jid'] = $jfdhhb['id'];
					$add_record['integral'] = $jfdhhb['integral'];
					$add_record['hongbao'] = $jfdhhb['hongbao'];
					$add_record['wechaname'] = $userinfo['wechaname'];
					$add_record['tel'] = $userinfo['tel'];
					$add_record['addtime'] = time();
					$id_jfdhhb_record = M('member_card_jfdhhb_record')->add($add_record);
					M('userinfo')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->setDec('total_score',$jfdhhb['integral']);
					$data['error'] = 0;
				}
			}
			...
			$this->assign('thisCard',$thisCard);
			$jfdhhb_list = M('member_card_jfdhhb')->where(array('token'=>$this->token,'cardid'=>intval($_GET['cardid']),'display'=>2,'starttime'=>array('lt',time())))->order('hongbao')->select();
			foreach($jfdhhb_list as $jk=>$jv){
				$jfdhhb_list[$jk]['mycount'] = M('member_card_jfdhhb_record')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'jid'=>$jv['id']))->count();
			}
			$this->assign('list',$jfdhhb_list);
			$this->display();
		}
	}
	public function wdhb(){
		$thisCard = M('member_card_set')->where(array('token'=>$this->token,'id'=>intval($_GET['cardid'])))->find();
		$this->assign('thisCard',$thisCard);
		$wdhb_list = M('member_card_jfdhhb_record')->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>intval($_GET['cardid'])))->order('addtime desc,id desc')->select();
		$this->assign('list',$wdhb_list);
		$this->display();
	}
}
?>