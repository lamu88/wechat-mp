...
class CollectwordAction extends WapAction{
	...
	public function index(){
		...
		if($share_key != ''){
			...
			if($is_my != ''){
				$this->redirect('Collectword/index',array('token'=>$this->token,'id'=>$id));exit;
			} else {
				...
				if (empty($share)) {
					...
					if (empty($is_prize) && $this->wecha_id) {
						if($this->info['is_help'] == 2 && !$this->isSubscribe()){
							$this->assign('firstShare', 'no');
						}else{
							...
							$this->assign('firstShare', 'yes');
						}
					}
				}
				/*
				 else {
					if (M('CollectwordUser')->where(array('token'=>$this->token,'pid'=>$id, 'wecha_id'=>$this->wecha_id))->getField('is_join')) {
						$this->redirect('Collectword/index',array('token'=>$this->token,'id'=>$id));exit;
					}
				}
				*/
		...
		$class = 'start';
		if ($my['is_prize']) $class =  'prize';
		if (empty($my['is_join'])) $class = 'in';
		if ($is_over == 2) $class = 'end';
		$this->assign('class', $class); // in start prize
		$this->display();
	}
	...
	public function share() {
		...
		$models = $model->alias('share')
		->where(array('share.token'=>$this->token, 'share.pid'=>$id, 'share.share_key'=>$_GET['key']))
		->join('LEFT JOIN '.C('DB_PREFIX').'userinfo AS user ON share.wecha_id = user.wecha_id AND share.token = user.token')
		->order('share.id DESC')
		->group('user.wecha_id')
		->field('share.*,user.wechaname,portrait')
		->limit(100)
		->select();
	...
	private function _count($id) {
		$count = M('CollectwordUser')->where(array('token'=>$this->token, 'pid'=>$id, 'is_prize'=>1))->count();
		return $count;
	}
	...
	private function _rank($limit) {
		...
		$result = $model->alias('cuser')
					->where(array('cuser.token'=>$this->token, 'cuser.pid'=>$id, 'cuser.is_join'=>1, 'cuser.word_count'=>array('neq', '0')))
					->join('LEFT JOIN '.C('DB_PREFIX').'userinfo AS user ON cuser.wecha_id = user.wecha_id AND cuser.token = user.token')
					->order('cuser.word_count DESC, cuser.share_num DESC,cuser.id ASC')
					->group('user.wecha_id')
					->field('cuser.*,user.wechaname,portrait')
					->limit($limit)->select();
		return $result;
	}
	...
	private function _prize() {
		...
		$model = M('CollectwordUser');
		$result = $model->alias('cuser')
		->where(array('cuser.token'=>$this->token, 'cuser.pid'=>$id, 'cuser.is_prize'=>1, 'cuser.is_join'=>1))
		->join('LEFT JOIN '.C('DB_PREFIX').'userinfo AS user ON cuser.wecha_id = user.wecha_id AND cuser.token = user.token')
		->order('cuser.id ASC')
		->group('user.wecha_id')
		->field('cuser.*,user.wechaname,portrait')
