...
class HelpingAction extends WapAction{
	...
	public function index(){
		...
		if($share_key != ''){
			...
			if(($myhelp['is_join2'] == 0 && $myhelp != '' && $myhelp['help_count'] > 1) || (intval($_GET['is_join2']) == 1 && $myhelp['is_join2'] == 0)){
		...
		if($this->helping['is_attention'] == 2 && !$this->isSubscribe()) {
			$this->memberNotice('',1);
		}else if(($this->helping['is_reg'] == 1 || $this->helping['is_sms'] == 1) && empty($this->fans['tel'])) {
		...
		}else if($this->helping['is_sms'] == 1 && empty($this->fans['tel']) && $this->fans['isverify'] != 1 && $this->helping['is_newtp'] == 1){
			$this->assign('sms',1);
			$this->assign('memberNotice','<div style="display:none"></div>');
		}
	...
	public function get_rank(){
		$where 	= array('token'=>$this->token,'pid'=>$this->helping['id'],'is_join2'=>1,'help_count'=>array('gt',0));
