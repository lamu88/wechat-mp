...
class HotelsAction extends WapAction {
	...
	public function index() {
		$company = M('Company')->where("`token`='{$this->token}' AND ((`isbranch`=0 AND `display`=1 AND business_type = '') OR (`isbranch`=1 AND `display`=1 AND `business_type` LIKE '%Hotels%'))")->select();
		if (count($company) == 1) {
			$this->redirect(U('Hotels/selectdate',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'cid' => $company[0]['id'])));
		}
		$price = M('Hotels_house_sort')->field('min(vprice) as price, cid')->group('cid')->where(array('token' => $this->token,'is_open'=>array('neq',2)))->select();
		$t = array();
	...
	public function hotel()
	{
		...
		$sorts = M('Hotels_house_sort')->where(array('cid' => $this->_cid, 'token' => $this->token,'is_open'=>array('neq',2)))->select();
		...
		if (IS_POST) {
			...
			for (; $time < $etime; ) {
				...
				if ($total > $hotelSort['houses']) {
					$this->error('您预定的房间数在'.date('Y年m月d日', $time).'超出总房间数了，请重新预定');
				} else {
					$time += 86400;
				}
			}
...
// 					}
					...
					$model = new templateNews();
					$model->sendTempMsg('TM00184', array('href' => U('Hotels/my',array('token' => $this->token, 'cid' => $this->_cid), true, false, true), 'wecha_id' => $this->wecha_id, 'first' => '预订房间提醒', 'ordertape' => date("Y年m月d日H时i分s秒"), 'ordeID' => $orderid, 'remark' => '预订房间成功，感谢您的光临，欢迎下次再次光临！'));
					if ($_POST['paymode'] == 4 && $this->fans['balance']) {
						$this->success('正在提交中...', U('CardPay/pay',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'from'=> 'Hotels', 'orderName' => $orderid, 'single_orderid' => $orderid, 'price' => $_POST['price'])));
					} elseif ($config['open']) {
						$this->success('正在提交中...', U('Alipay/pay',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'from'=> 'Hotels', 'orderName' => $orderid, 'single_orderid' => $orderid, 'price' => $_POST['price'])));
					} else {
						$this->success('预定成功,进入您的订单页', U('Hotels/my',array('token' => $this->token, 'cid' => $this->_cid)));
					}
				} else {
					$this->error('操作失败');
				}
	...
	public function my()
	{
		$this->_cid || $this->_cid = isset($_GET['cid']) ? intval($_GET['cid']) : 0;
		$this->_cid && $_SESSION["session_hotel_{$this->token}"] = $this->_cid;
		$company = M('Company')->where(array('id' => $this->_cid, 'token' => $this->token))->find();
		$orders = M('Hotels_order')->where(array('cid' => $this->_cid, 'token' => $this->token, 'wecha_id' => $this->wecha_id, 'status' => array('elt', 2)))->order('id desc')->limit($this->offset)->select();
		$list = array();
		foreach ($orders as $o) {
		...
		$count = M('Hotels_order')->where(array('cid' => $this->_cid, 'token' => $this->token, 'wecha_id' => $this->wecha_id, 'status' => array('elt', 2)))->count();
	...
	public function ajaxorder()
	{
		...
		$orders = M('Hotels_order')->where(array('cid' => $this->_cid, 'token' => $this->token, 'wecha_id' => $this->wecha_id, 'status' => array('elt', 2)))->order('id desc')->limit($start . ', ' . $this->offset)->select();
		$list = array();
		foreach ($orders as $o) {
			$o['day'] = (strtotime($o['enddate']) - strtotime($o['startdate'])) / 86400;
			$o['startdate'] = date("m月d日", strtotime($o['startdate']));
	...
	public function payReturn() {
		if(isset($_GET['nohandle'])){
			//执行跳转
			$this->success('预定成功,进入您的订单页', U('Hotels/my',array('token' => $this->token, 'cid' => $this->_cid, 'twid' => $this->_twid)));
// 			$this->redirect(U('Hotels/my', array('token' => $this->token,'wecha_id' => $this->wecha_id, 'twid' => $this->_twid)));
		}else {
			$out_trade_no = $_GET['orderid'];
			ThirdPayHotels::index($out_trade_no);
		}
