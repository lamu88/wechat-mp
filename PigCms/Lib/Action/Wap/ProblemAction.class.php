...
class ProblemAction extends WapAction{
	...
	public function index(){
		if($this->pro_info['is_follow'] == 2 && $this->isSubscribe() == false){
			$this->assign('is_follow','no');
			$this->memberNotice('请先关注我们的公众号',1);
		}
	...
	public function users(){
		...
		if($this->_get_play_day($this->pro_info['start_time'])  > $this->pro_info['end_day']){
			$this->error('游戏已过'.$this->pro_info['end_day'].'天，欢迎下次光临',U('Problem/question_status',array('token'=>$this->token,'id'=>$id)));
			exit();
		}
		//已有信息跳转答题
		if($user){
			header("Location:".U('Problem/question_status',array('token'=>$this->token,'id'=>$id)));
			exit();
		}
	...
	public function requestion(){
		...
		if($this->_get_play_day($this->pro_info['start_time'])  > $this->pro_info['end_day']){
			$this->error('游戏已过'.$this->pro_info['end_day'].'天，欢迎下次光临',U('Problem/question_status',array('token'=>$this->token,'id'=>$id)));
			session('question_id_session_'.$uid,null);
			exit();
		}

		if(empty($user['id'])){
			$this->error('请先填写个人信息',U('Problem/users',array('token'=>$this->token,'id'=>$id)));
			session('question_id_session_'.$uid,null);
			exit();
		}

		if($this->_ck_status($id,$user['id'])){
			$this->error('今日答题已结束',U('Problem/question_status',array('token'=>$this->token,'id'=>$id)));
			session('question_id_session_'.$uid,null);
			exit();
		}
	...
	public function question_status(){
		...
		if($data['play_day'] > $this->pro_info['end_day']){
			...
			//如果今天已经开始答题直接跳转答题界面
			if($qCount != 0){
				header("Location:".U('Problem/requestion',array('token'=>$this->token,'id'=>$id)));
				exit();
			}
	...
	*/
	public function _rand_question($id,$uid){
		...
		$question_id_session = session('question_id_session_'.$uid);
		if(count($question_id_session) >= $this->pro_info['question_num']){
			session('question_id_session_'.$uid,null);
			$question_id_session = null;
		}
		if(in_array($id_arr[$key],$question_id_session)){
			$this->_rand_question($id,$uid);exit;
		}else{
			$question_id_session[] = $id_arr[$key];
			session('question_id_session_'.$uid,$question_id_session);
		}
		$id 	= $id_arr[$key];
