...
// 		$this->token = isset($_REQUEST['token']) ? htmlspecialchars($_REQUEST['token']) :  '';//$this->_get('token');
	...
	public function answer()
	{
		...
		if (empty($que)) {
			$this->success("参加完毕，现在进行自动抽奖", U("Research/lotter", array('reid' => $this->_rid, 'token' => $this->token)));
		}else{
			$this->display();
		}  
	}
	...
	public function lotter()
	{
		...
		if ($this->_research['starttime'] > time()) {
			$this->error('活动未开始，还不能抽奖', U("Research/index", array('reid' => $this->_rid, 'token' => $this->token)));
		} elseif ($this->_research['endtime'] < time()) {
			$this->error('活动已结束，下次来早点吧', U("Research/index", array('reid' => $this->_rid, 'token' => $this->token)));
		}
		
		if (empty($id)) {
			$this->redirect(U("Research/index", array('reid' => $this->_rid, 'token' => $this->token)));
		}
		...
		if ($this->wecha_id&&!$this->fans&&$Lottery['needreg']){
			$this->error('请先完善个人资料再参加活动',U('Userinfo/index',array('token'=>$this->token,'redirect'=>MODULE_NAME.'/index|id:'.intval($id))));
		}
