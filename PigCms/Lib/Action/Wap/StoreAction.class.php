...
// 		}
	...
	public function select()
	{
		...
		if (count($company) == 1) {
			$this->redirect(U('Store/index',array('token' => $this->token, 'cid' => $company[0]['id'], 'twid' => $this->_twid)));
		}
	...
	{
		$this->cats();
// 		$this->redirect(U('Store/cats',array('token' => $this->token, 'twid' => $this->_twid, 'cid' => $this->_cid)));
	}
	...
	public function cats() 
	{
		...
		if ($this->_isgroup) {
			...
			if (empty($relation) && $this->_cid != $cid) {
				$this->error("该店铺暂时没有商品可卖，先逛逛别的", U('Store/select',array('token' => $this->token, 'twid' => $this->_twid)));
			}
		}
		...
		foreach ($cats as &$row) {
			...
			if ($row['isfinal'] == 1) {
				...
				$row['sub'] = array();
				$row['url'] = U('Store/index', array('token' => $this->token, 'cid' => $this->_cid, 'parentid' => $row['id'], 'twid' => $this->_twid));
			}
			$info[$row['id']] = $row;
		...
		$catemenu[0] = array('id' => 0, 'name' => '所有商品', 'picurl' => '/tpl/static/store/m-act-cat.png', 'k' => 0, 'vo' => array(), 'url' => U('Store/products', array('token'=> $this->token,'cid' => $this->_cid)));
		$catemenu[1] = array('id' => 1, 'name' => '购物车', 'picurl' => '/tpl/static/store/m-act-cart.png', 'k' => 1, 'vo' => array(), 'url' => U('Store/cart', array('token'=> $this->token,'cid' => $this->_cid)));
		$catemenu[2] = array('id' => 2, 'name' => '查物流', 'picurl' => '/tpl/static/store/m-act-wuliu.png', 'k' => 2, 'vo' => array(), 'url' => U('Store/my', array('token'=> $this->token,'cid' => $this->_cid)));
		$catemenu[3] = array('id' => 3, 'name' => '用户中心', 'picurl' => '/tpl/static/store/user2.png', 'k' => 3, 'vo' => array(), 'url' => U('Store/myinfo', array('token'=> $this->token,'cid' => $this->_cid)));
	...
	public function products() 
	{
		...
		if (IS_POST){
            ...
            $this->redirect('/index.php?g=Wap&m=Store&a=products&token=' . $this->token . '&keyword=' . $key . '&twid=' . $this->_twid);
		}
...
// 		$products = $this->product_model->where($where)->order("sort ASC, " . $order.' '.$method)->limit($start . ',' . $pageSize)->select();
	...
	public function product() 
	{
		...
		if (empty($product)) {
			$this->redirect(U('Store/products',array('token' => $this->token, 'twid' => $this->_twid)));
		}
...
// 		}
	...
	public function deleteCart()
	{
		...
		$_SESSION[$this->session_cart_name] = serialize($carts);
		$this->redirect(U('Store/cart',array('token'=>$_GET['token'], 'twid' => $this->_twid)));
	}
	...
	public function ordersave()
	{
		...
		if ($carts){
			...
			foreach ($carts as $pid => $rowset) {
				$total = 0;
				$tmp = M('product')->where(array('id' => $pid, 'status' => 0))->find();//setDec('num', $total);
				if (is_array($rowset)) {
		...
		if ($normal_rt){
			...
			if (empty($cartObj)) {
				...
				if ($_POST['saveinfo']) {
					...
					$userRow = array('address' => $row['address']);
					if ($thisUser) {
...
// 					$userinfo_model->where(array('id' => $thisUser['id']))->save($userRow);
			...
			if ($totalprice) {
				...
				ThirdPayStore::index($orderid);
			}
			$this->success('预定成功,进入您的订单页', U('Store/my',array('token' => $_GET['token'], 'success' => 1, 'twid' => $this->_twid)));
		} else {
			$this->error('订单生产失败');
		} 
	}
...
// 		$this->assign('alipayConfig', $alipayConfig);
		...
		if (empty($data[0])) {
			$this->redirect(U('Store/cart', array('token' => $this->token, 'twid' => $this->_twid)));
		}
		if (isset($data[1])) {
			foreach ($data[1] as $pid => $row) {
				$totalCount += $row['total'];
		...
		if (empty($totalCount)) {
			$this->error('没有购买商品!', U('Store/cart', array('token' => $this->token, 'twid' => $this->_twid)));
		}
	...
	public function commentSave()
	{
		...
		if (false !== $comment->create($data)) {
			unset($data['__hash__']);
			$comment->add($data);
			$this->success("评论成功", U('Store/myDetail',array('token' => $this->token,'cartid' => $cartid, 'twid' => $this->_twid)));
		} else {
			$this->error($comment->error, U('Store/myDetail',array('token' => $this->token,'cartid' => $cartid, 'twid' => $this->_twid)));
		}
	}
	public function deleteOrder()
	{
		...
		$this->redirect(U('Store/my', array('token' => $_GET['token'], 'twid' => $this->_twid)));
	}
...
//                 $uid = $userInfo['id'];
	...
	{
        if (session('twid')) {
            $this->redirect(U('Store/myinfo', array('token' => $this->token, 'twid' => $this->mytwid)));
        }
		...
		if (IS_POST) {
			...
			if (empty($userInfo)) {
				...
				if ($callbackurl) {
					...
					$this->success('登录成功', U('Store/index', array('token' => $this->token, 'twid' => $this->_twid)));
				}
			}
		} else {
...
// 			} else {//点击
				...
				$twhere = "token='{$token}' AND cid='{$cid}' AND twid='{$twid}' AND type='{$type}' AND dateline>'{$stime}' AND dateline<'{$etime}'";
				$count = $db->where($twhere)->sum('param');
...
// 			}
			...
			$params['template']['template_data']['href'] = U('Store/detail',array('token' => $token, 'twid' => $twid), true, false, true);
...
// 		echo $this->mytwid;die;
	...
	public function logout()
	{
		...
		$this->redirect(U('Store/cats',array('token' => $this->token, 'twid' => $this->_twid)));
	}
	...
	public function setremove()
	{
		if ($this->mytwid) {
			...
			if (IS_POST) {
				...
				if ($total < $price) {
					$this->error("请不要贪心，您现在还没有{$price}元钱供你提现");
				}
