...
class Commonpage{
    ...
    public function show($num=0){
		...
		/*$str = '<span class="total"><span id="row_count">'.$now.' / '.$total.' 页  </span>&nbsp;&nbsp;<div class="pages">';*/
		$str = '<div class="pages">';
		if($now > 1){
			$str .= '<a href="'.$url.($now-1).'" class="pga" title="上一页"> < </a>';
		}
		...
		if ($now != $total){
			$str .= '<a href="'.$url.($now+1).'" class="pga"> > </a>';
		}
		$str.='</div>';
		return $str;
    }
