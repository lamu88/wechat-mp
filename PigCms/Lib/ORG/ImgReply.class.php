...
class ImgReply {	
	...
	public function __construct($token,$wechat_id,$data,$siteUrl,$key) {
		...
		if ('img' == strtolower($data2['module']) && 2 < $this->thisWxUser['winxintype']) { // 3 4 为认证号
			$groupid = $this->getGroupId();
			$like2 = "( concat(',',wechat_group,',') LIKE '%,$groupid,%' OR wechat_group = '' ) ";
			if ($like['precisions']) {
				//找出关键词表中满足条件的pid
				$pids = "";
				$pidsArr = M('keyword')->where(array('keyword'=>$like['keyword'],'token'=>$like['token'],'module'=>'Img'))->field('pid')->select();
				foreach ((array)$pidsArr as $key => $value) {
					$pids .= $value['pid'].',';
				}
				if(trim($pids,',') != ""){ $like2 = "id in (".trim($pids,',').")";}
			} else {
