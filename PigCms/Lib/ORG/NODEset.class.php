...
class NODEset{
	public function index(){
		...
		if($genjiedian_Site_Site_weixin_pay == ''){
			$genjiedian_Site_Site_weixin_pay = M('Node')->add(array('title'=>'微信支付服务商','name'=>'weixinPay','pid'=>intval($genjiedian_Site_Site),'level'=>3,'status'=>1,'display'=>2));
		}
		...
		if($rollback == ''){//首页(System)
				$genjiedian_Site_System = M('Node')->where(array('name'=>'System','pid'=>intval($genjiedian_Site),'level'=>2,'status'=>1,'display'=>0))->getField('id');
					...
					//站点管理 (Site)->首页 (System)->【添加】回滚 (Index)(隐藏)
					...
					if($genjiedian_Site_System_Indexhg == ''){
						...
						//站点管理 (Site)->首页 (System)->回滚 (Index)->【添加】回滚程序 (rollback)(隐藏)
					...
					//站点管理 (Site)->首页 (System)->【添加】升级程序 (checkUpdate)(隐藏)
					...
					//站点管理 (Site)->首页 (System)->【添加】升级数据库 (doSqlUpdate)(隐藏)
