...
class WechatCoupons extends apiOauth
{
	...
	public function addCompany($company = array()){
		...
		//$photo_res['url'] = 'http://mmbiz.qpic.cn/mmbiz/xuHsx350294HQXBtj39P9re9uBJQUaae07BdUHPzakHVibVVLASu70mEficU8IBfQk6IeovgWzfGjcjeCSlCuA7g/0';
		if(strpos($company['opentime'],':') === FALSE){
			$company['opentime'] = $company['opentime'].':00';
		}
		if(strpos($company['closetime'],':') === FALSE){
			$company['closetime'] = $company['closetime'].':00';
		}
	...
	//修改门店
	public function updatepoi($company = array()){
		if(empty($company)){ return false;}
	...
	public function createCard($id,$is_edit=0){
		...
		$company[0]['intro'] = strip_tags(html_entity_decode($company[0]['intro']));
		switch ($info['type']) {
			case '0':
