...
if(strtolower($_SERVER['REQUEST_METHOD']) == 'post'){
	...
	{
		$config = require('../Conf/info.php');
	}
	else if(file_exists('../PigData/conf/info.php'))
	{
		$config = require('../PigData/conf/info.php');
	}
	else
	{
		$config = require('../DataPig/conf/info.php');
	}

	$username = trim($_POST['phone']);
    $password = trim($_POST['password']);
	
	$salt = !empty($config['encryption']) ? $config['encryption'] : 'pigcms';
	$post_data = array();
	...
	$domain = !empty($config['weidian_domain']) ? $config['weidian_domain'] : 'http://v.meihua.com';
	$url = $domain . '/api/login.php';
