var menu = {
	...
	init: function(id){
		...
		$('.menu_tt').each(function(){
			/*console.log($(this).offsetParent().attr('class')+' '+this.offsetParent.tagName+' => '+$(this).offset().top);*/
	...
	regetoffsetAry: function(){
		var gthis=this;
		$('.menu_tt').each(function(i){
			var otop=$(this).offset().top;
			if(i>3){
				if(gthis.offsetAry[i]!=otop){
					gthis.offsetAry.push[i]=otop;
				}
...
$(function(){
	...
	$('.menu_list li').click(function(e){
		...
		if(F('.add').length){
			$('#detailBtn').removeClass('disabled').text('来'+unit);
		}else{
